...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div
      className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 shrink-0 ${colorClass}`}
    >
      {initials}
    </div>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {users.map((u) => {
          ...
          const spentPct = hasAlloc
            ? Math.min((u.totalExpense / u.alloc) * 100, 100)
            : 0;
          ...
          return (
            <div
              key={u.userId}
              className="rounded-2xl bg-white border border-cream-200 shadow-sm p-5 flex flex-col gap-4"
            >
              {/* Header */}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-emerald-50 border border-emerald-100 px-3 py-2.5">
                  ...
                  <p className="text-sm font-bold text-emerald-700">
                    {formatRupiah(u.totalIncome)}
                  </p>
                </div>
                <div className="rounded-xl bg-red-50 border border-red-100 px-3 py-2.5">
                  <p className="text-[10px] text-red-500 uppercase tracking-wider font-medium mb-0.5">
                    Pengeluaran
                  </p>
                  <p className="text-sm font-bold text-red-600">
                    {formatRupiah(u.totalExpense)}
                  </p>
                </div>
              </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="text-warm-500">
                      {isOver ? "⚠️ Melebihi alokasi" : "Sisa alokasi"}
                    </span>
                    <span
                      ...
                      <div key={idx} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            <span className="text-xs text-warm-700 truncate">
                              {cat.categoryName}
                            </span>
              ...
              {/* Empty state */}
              {u.topCategories.length === 0 && (
                <p className="text-xs text-warm-300 italic text-center py-2">
